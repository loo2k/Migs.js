...
function _mimeVerify(ext, type) {
    var extInfo = exports.mimes[ext];
    if(extInfo) {
        if(extInfo.constructor === String && extInfo === type) {
            return true;
        } else if(extInfo.constructor === Array) {
...
exports.mimesVerify = function(exts, type) {
    if(exts.constructor == Array) {
        for(var i = 0; i < exts.length; i++) {
            if(_mimeVerify(exts[i], type)) {
                return true;
            }
        }
    } else {
        if(_mimeVerify(exts, type)) {
            return true;
        }
    }
    return false;
